...
module.exports = function (_collectionName) {
   ...
   *
   * @param {collection to run} _collectionName
   * @param {pass true from fun call if wants failed test report} onlyShowFailed
   * @param {pass true from fun call if wants normal newman report} normalReportGenerated
   */
  this.run = function (
    _collectionName,
    onlyShowFailed = false,
    normalReportGenerated = false
  ) {
    return new Promise((resolve, reject) => {
      newman
        .run(
          {
            collection: require("../collections/Cord_API_V3_" +
              _collectionName +
          ...
          function (err) {
            ...
            console.log(
              "\u001b[" +
                ...
                "\nBooooooom... " +
                _collectionName +
                " collection newman report is ready.\n" +
                "\u001b[0m"
            );
          }
        )
        .on("start", function (err, args) {
          ...
          console.log(
            changeColor("\nWoohoo... collection ", 31) +
              _collectionName +
              changeColor(" is runnning...\n", 31)
          );
 ...
 * @param {colorCode to apply} colorCode
...
function commands() {
  /**
   * Use Below Command to run collection from CLI
   */
  ...
  /**
   * Run JS file with increased Memory
   */
